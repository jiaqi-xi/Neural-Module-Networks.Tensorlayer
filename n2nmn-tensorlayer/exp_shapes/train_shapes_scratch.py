...
# Training parameters
invalid_expr_loss = 3  # loss value when the layout is invalid
lambda_entropy = 0.5
weight_decay = 5e-4
baseline_decay = 0.99
max_grad_l2_norm = 10
...
# Loss function
#softmax_loss_per_sample = tf.nn.sparse_softmax_cross_entropy_with_logits(#tl.cost.cross_entropy_seq_with_mask 计算交叉熵
#    logits=scores, labels=vqa_label_batch)
#################
softmax_loss_per_sample  = tl.cost.cross_entropy(scores,vqa_label_batch)
...
total_loss = tf.add_n([total_training_loss,
                       lambda_entropy * nmn3_model.entropy_reg,
...
gradients = [
    (tf.clip_by_norm(g, max_grad_l2_norm), v) for g, v in gradients]
...
tf.summary.scalar("baseline", baseline_ph)
tf.summary.scalar("validity", validity_ph)
